...
\subsection{Tables}
Initially, only one table was needed to make the database capable of doing what the storage manager program could do. This table logged and kept track of the current state of the slots on the storage disk. However, to make the database more useful, two additional tables were created: one table for logging the current object detected by the vision system, and one table for logging the history.
