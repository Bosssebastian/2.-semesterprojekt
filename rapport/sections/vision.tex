...
When trying to find an object in a picture, people look for features that make the object easy to identify. This is often something like the outline of the object. However, if only the edge of an object is used, the detected point can be placed anywhere along that edge. A corner is therefore more useful, because if it is moved slightly, it is easier to see that it is no longer in the correct place. \cite{opencv_features}
