...
class dataLogger
{
    public:
        ...
        void appendTarget(int target);
        void calculateMovingDistance(); // if it need to go from 8-4, that is a distance of 4, each possible moving distance is stored in vector
    ...
    private:
        ...
        int mSuccess = 0;
        int mIrregularities = 0;
        float mStart;
        std::vector<int> mStoredTargets;
        std::vector<float> mDeltaTime;
        std::vector<int> mStoredPositions;
        std::vector<int> mDeltaMove;
};
